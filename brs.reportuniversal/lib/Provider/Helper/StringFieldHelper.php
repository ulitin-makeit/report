...
/**
 * Хелпер для работы с пользовательскими полями типа "строка" (string), "число" (integer), "дата" (date) и "дата/время" (datetime)
...
class StringFieldHelper
{
	...
	public function loadFieldData(string $fieldCode, array $fieldInfo): array
	{
		...
		if (!in_array($fieldInfo['type'], ['string', 'integer', 'datetime', 'date'])) {
			throw new ReportException("Неподдерживаемый тип поля: {$fieldInfo['type']}. Ожидается string, integer, date или datetime.");
		}
